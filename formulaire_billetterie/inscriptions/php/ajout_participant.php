...
if(isset($_POST))
{
    ...
    if(isset($_POST['icam_informations']))
    {
        ...
        {
            echo "Le quota pour les " . $promo . " de " . $site . " est déjà plein. ";
            die();
        }
        ...
        if($icam_data!=false)
        {
            ...
            if(isset($_POST['guests_informations']))
            {
                ...
                if($guests_data!=false)
                {
                    ...
                    var_dump(get_current_participants_number($event_id) + $participant_additions);
                    var_dump($event['total_quota']);
                    ...
                    if(get_current_promo_quota(array("event_id" => $event_id, "promo_id" => get_promo_id('Invités'), "site_id" => $site_id)) > $guests_specifications['quota'])
                    {
    ...
    {
        echo "Quelqu'un s'est débrouillé pour supprimer l'input hidden de nom 'total_transaction_price'";
        die();
    }
