...
/**
 * Template d'une option.
 * La fonction peut être appelée directment en serveur :
 * dans le cas de l'edit, ou on met tout de suite les options dans le formulaire.
 * Elle est sinon appelée en Ajax lorsque l'on clique sur le + du formulaire à côté des options.
 */
        ...
        <div id=<?= '"option_'.$option_number.'"'; ?> class="panel-collapse collapse panel-body">
            <div class="option_generalities">
                ...
                <div class="form-check" aria-describedby=<?= '"option_'.$option_number.'_accessibility_help"'; ?> >
                    <label class="radio-inline"><input <?= isset($option['everyone_has_option']) ? ($option['everyone_has_option']==1 ? 'checked' : '') : '' ?> type="radio" class="option_accessibility_input" name=<?= '"option_'.$option_number.'_accessibility"'; ?> value=1 required>Oui</label>
                    <label class="radio-inline"><input <?= isset($option['everyone_has_option']) ? ($option['everyone_has_option']==0 ? 'checked' : '') : '' ?> type="radio" class="option_accessibility_input" name=<?= '"option_'.$option_number.'_accessibility"'; ?> value=0>Non</label>
                </div>
            ...
            <div class="option_type_complement">
                ...
                <div class="select_type">
                    ...
                    <div class="form-check" aria-describedby=<?= '"select_option_'.$option_number.'_is_mandatory_help"'; ?> >
                        <label class="radio-inline"><input <?= isset($option['type']) ? ($option['type']=='Select' ? ($option['is_mandatory']==1 ? 'checked' : '') : '') : '' ?> type="radio" class="select_option_mandatory_input" name=<?= '"select_option_'.$option_number.'_is_mandatory"'; ?> value=1>Oui</label>
                        <label class="radio-inline"><input <?= isset($option['type']) ? ($option['type']=='Select' ? ($option['is_mandatory']==0 ? 'checked' : '') : '') : '' ?> type="radio" class="select_option_mandatory_input" name=<?= '"select_option_'.$option_number.'_is_mandatory"'; ?> value=0>Non</label>
                    </div>
            ...
            <div class="option_accessibility">
                ...
                <table class="table option_accessibility_table">
                    ...
                    <tbody>
                    <?php isset($promo_options) ? insert_option_accessibility_rows($promo_options) : '' ?>
                    </tbody>
                </table>
...
// Si elle est appelée en Ajax, on l'affiche ici
